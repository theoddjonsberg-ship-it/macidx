-- MII calculation + Trust score recalculation
-- This function recalculates mii_level and trust_score based on machine data
...
REVOKE ALL ON FUNCTION public.recalc_machine_mii_and_trust(uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.recalc_machine_mii_and_trust(uuid) TO authenticated;

-- ============================================================
-- TRIGGER FUNCTIONS
-- ============================================================

-- Guarded trigger for machines (avoids recursion)
...
-- Trigger for related tables that have machine_id
CREATE OR REPLACE FUNCTION public.trg_recalc_via_machine_id()
RETURNS trigger
LANGUAGE plpgsql
AS $$
BEGIN
  IF TG_OP = 'DELETE' THEN
    IF OLD.machine_id IS NOT NULL THEN
      PERFORM public.recalc_machine_mii_and_trust(OLD.machine_id);
    END IF;
  ELSE
    IF NEW.machine_id IS NOT NULL THEN
      PERFORM public.recalc_machine_mii_and_trust(NEW.machine_id);
    END IF;
  END IF;
...
-- Trigger on machines
DROP TRIGGER IF EXISTS trg_machines_recalc ON public.machines;
CREATE TRIGGER trg_machines_recalc
AFTER INSERT OR UPDATE ON public.machines
FOR EACH ROW EXECUTE FUNCTION public.trg_recalc_machine_guarded();

-- Trigger on documents
DROP TRIGGER IF EXISTS trg_documents_recalc ON public.documents;
CREATE TRIGGER trg_documents_recalc
AFTER INSERT OR UPDATE OR DELETE ON public.documents
FOR EACH ROW EXECUTE FUNCTION public.trg_recalc_via_machine_id();

-- Trigger on machine_events
DROP TRIGGER IF EXISTS trg_machine_events_recalc ON public.machine_events;
CREATE TRIGGER trg_machine_events_recalc
AFTER INSERT ON public.machine_events
FOR EACH ROW EXECUTE FUNCTION public.trg_recalc_via_machine_id();

-- Trigger on machine_verifications
DROP TRIGGER IF EXISTS trg_machine_verifications_recalc ON public.machine_verifications;
CREATE TRIGGER trg_machine_verifications_recalc
AFTER INSERT OR UPDATE ON public.machine_verifications
FOR EACH ROW EXECUTE FUNCTION public.trg_recalc_via_machine_id();

-- Trigger on gps_machines
DROP TRIGGER IF EXISTS trg_gps_machines_recalc ON public.gps_machines;
CREATE TRIGGER trg_gps_machines_recalc
AFTER INSERT OR UPDATE OR DELETE ON public.gps_machines
FOR EACH ROW EXECUTE FUNCTION public.trg_recalc_via_machine_id();

-- ============================================================
-- BACKFILL EXISTING MACHINES
-- ============================================================
